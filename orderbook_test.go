...
func addDepth(ob *OrderBook, prefix string, quantity decimal.Decimal) {
	for i := 50; i < 100; i = i + 10 {
		ob.ProcessLimitOrder("test", Buy, fmt.Sprintf("%sbuy-%d", prefix, i), quantity, decimal.New(int64(i), 0))
	}

	for i := 100; i < 150; i = i + 10 {
		ob.ProcessLimitOrder("test", Sell, fmt.Sprintf("%ssell-%d", prefix, i), quantity, decimal.New(int64(i), 0))
	}
...
func TestLimitPlace(t *testing.T) {
	...
	for i := 50; i < 100; i = i + 10 {
		done, partial, partialQty, err := ob.ProcessLimitOrder("test", Buy, fmt.Sprintf("buy-%d", i), quantity, decimal.New(int64(i), 0))
		if len(done) != 0 {
	...
	for i := 100; i < 150; i = i + 10 {
		done, partial, partialQty, err := ob.ProcessLimitOrder("test", Sell, fmt.Sprintf("sell-%d", i), quantity, decimal.New(int64(i), 0))
		if len(done) != 0 {
			t.Fatal("OrderBook failed to process limit order (done is not empty)")
		}
		if partial != nil {
...
func TestLimitProcess(t *testing.T) {
	...
	done, partial, partialQty, err := ob.ProcessLimitOrder("test", Buy, "order-b100", decimal.New(1, 0), decimal.New(100, 0))
	if err != nil {
		t.Fatal(err)
	}
	...
	done, partial, partialQty, err = ob.ProcessLimitOrder("test", Buy, "order-b150", decimal.New(10, 0), decimal.New(150, 0))
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, _, _, err := ob.ProcessLimitOrder("test", Sell, "buy-70", decimal.New(11, 0), decimal.New(40, 0)); err == nil {
		t.Fatal("Can add existing order")
	}

	if _, _, _, err := ob.ProcessLimitOrder("test", Sell, "fake-70", decimal.New(0, 0), decimal.New(40, 0)); err == nil {
		t.Fatal("Can add empty quantity order")
	}

	if _, _, _, err := ob.ProcessLimitOrder("test", Sell, "fake-70", decimal.New(10, 0), decimal.New(0, 0)); err == nil {
	...
	done, partial, partialQty, err = ob.ProcessLimitOrder("test", Sell, "order-s40", decimal.New(11, 0), decimal.New(40, 0))
	if err != nil {
		t.Fatal(err)
	}
...
func TestMarketProcess(t *testing.T) {
	...
	done, partial, partialQty, left, err := ob.ProcessMarketOrder("test", Buy, decimal.New(3, 0))
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, _, _, _, err := ob.ProcessMarketOrder("test", Buy, decimal.New(0, 0)); err == nil {
		t.Fatal("Can add zero quantity order")
	}

	done, partial, partialQty, left, err = ob.ProcessMarketOrder("test", Sell, decimal.New(12, 0))
	if err != nil {
		t.Fatal(err)
	}
...
func BenchmarkLimitOrder(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		ob.ProcessLimitOrder("test", Buy, "order-b150", decimal.New(160, 0), decimal.New(150, 0)) // 1 ts
		ob.ProcessMarketOrder("test", Sell, decimal.New(200, 0))                                  // 1 ts = total 32
	}
